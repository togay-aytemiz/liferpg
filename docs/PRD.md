...
- User-forged daily quests should remain **always visible** on top of the rotating AI daily focus set instead of replacing one of the AI-selected visible dailies.
...
- **2026-03-11 - Weekly Offer Rotation Messaging:** Personalized Bazaar offers now expose their 7-day rotation at the section level instead of per-card. The Bazaar header shows one shared countdown to the current offer refresh, and opening the Bazaar after expiry should trigger the next weekly offer set instead of leaving stale expired inventory visible.
- **2026-03-11 - Weekly Offer App-Day Cadence:** Personalized Bazaar offers no longer expire on a floating `now + 7 days` timer. Their weekly cadence is now anchored to the same `03:00 -> 03:00` app-day system as dailies, so each offer batch refreshes at the next `03:00` boundary after 7 app days and the Bazaar can swap in a new batch as soon as the player opens it after that boundary.
...
- **2026-03-11 - Quest Description Clamp:** Shared quest cards now allow up to three lines of body copy before truncation. This preserves short explanatory context for medium-length daily/side/boss tasks without reopening the card layout.
