...
serve(async (req) => {
    ...
    try {
        ...
        const { data: profile } = await supabase
            .from("profiles")
            .select("likes, dislikes, focus_areas, ai_weekly_focus, ai_weekly_focus_generated_at, level, stat_strength, stat_knowledge, stat_wealth, stat_adventure, stat_social")
            .eq("id", user.id)
            .single();
